	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, obj := range tt.srcObjects {
				...
				ns := obj.GetNamespace()
				_, err = srcClient.Resource(gvr).Namespace(ns).Create(context.Background(), obj, metav1.CreateOptions{})
				assert.NoError(t, err)
			}
			...
			for _, want := range tt.importedObjects {
				...
				got, err := destClient.Resource(gvr).Namespace(ns).Get(context.Background(), want.GetName(), metav1.GetOptions{})
				assert.NoError(t, err)
			...
			for _, notWant := range tt.notImportedObjects {
				...
				assert.NoError(t, err)
				ns := notWant.GetNamespace()
				got, err := destClient.Resource(gvr).Namespace(ns).Get(context.Background(), notWant.GetName(), metav1.GetOptions{})
				assert.Error(t, err)
				assert.Nil(t, got)
			}
		})
